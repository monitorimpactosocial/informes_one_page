...
// ── CRYPTO: SHA-256 en cliente (para envío de contraseña) ────
const Crypto = (() => {
...
const API = (() => {
  ...
  async function request(action, method = 'GET', body = null) {
    ...
    const opts = { method };
    if (method === 'POST' && body) {
      // Incluir token en body también (fallback para Apps Script)
    ...
    // Apps Script siempre retorna 200; leemos __status del payload
  ...
  return {
    login: (usuario, hashPwd) => request('login', 'POST', { usuario, password: hashPwd }),
    me:    ()             => get('me'),
...
const Auth = (() => {
  async function login(usuario, password) {
    // Hashear password en cliente antes de enviar
    const hashPwd = await Crypto.sha256(password);
    const data    = await API.login(usuario, hashPwd);
    State.saveSession({
